const app = {
  ...
  open: function ({ user, members, requests, ranks }) {
    app.ranksOfRole = ranks[user.org];
    ...
    if (['coronelpm', 'tenencoronelpm', 'comandantebope', 'coronelbope', 'marechalexercito', 'generalexercito', 'delegadogeral', 'delegado', 'coronelcore', 'uniopecore', 'adminlotusgroup@445', 'admin'].filter((u) => user.role.toLowerCase().includes(u)).length === 0) {
      if (!user.isAdmin) {
