...
vRPserver = Tunnel.getInterface("vRP","vrp_legacyfuel")

src = {}
Tunnel.bindInterface("vrp_legacyfuel",src)
vSERVER = Tunnel.getInterface("vrp_legacyfuel")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local lastFuel = 0
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(ped) then
			...
			if IsVehicleEngineOn(vehicle) and GetPedInVehicleSeat(vehicle,-1) == ped and speed >= 10 and GetVehicleFuelLevel(vehicle) >= 2 then
				SetVehicleFuelLevel(vehicle,GetVehicleFuelLevel(vehicle)-0.4)
				--TriggerServerEvent("vrp_engine:tryFuel",VehToNet(vehicle),GetVehicleFuelLevel(vehicle)-0.4)
			end
		end
					...
					if distance <= 3.5 then
						...
						if not isFuel then
							...
							if vehFuel >= 0.0 and GetAmmoInPedWeapon(ped,883325847) - 0.02 * 100 > 0 then
								...
								if vehFuel >= 100.0 or GetAmmoInPedWeapon(ped,883325847) - 0.02 * 100 <= 0 or GetEntityHealth(ped) <= 105 then
						...
						if IsControlJustPressed(1,38) and segundos == 0 then
							segundos = 3
							if isFuel then
								TriggerServerEvent("vrp_engine:tryFuel",VehToNet(vehicle),vehFuel)
								StopAnimTask(ped,"timetable@gardener@filling_can","gar_ig_5_filling_can",2.0)
							...
							if distance <= 3.5 then
								...
								if not isFuel then
									...
									if vehFuel >= 0.0 then
										...
										isGallons = isGallons + 0.08
										SetVehicleFuelLevel(vehicle,vehFuel+0.08)
										text3D(coordsVeh.x,coordsVeh.y,coordsVeh.z+1,"~g~E~w~   CANCELAR")
										...
										if vehFuel >= 100.0 or GetEntityHealth(ped) <= 105 then
...
function src.syncAdminFuel(index,fuel)
	vehFuels[VehToNet(index)] = fuel + 0.0
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- SYNCFUELPLAYERS
-----------------------------------------------------------------------------------------------------------------------------------------
...
function criarBlip()
    for _, item in pairs(fuelLocs) do
		item.blip = AddBlipForCoord(item[2],item[3],item[4])
		SetBlipSprite(item.blip, 361)
		SetBlipColour(item.blip, 35)
		SetBlipScale(item.blip, 0.3)
