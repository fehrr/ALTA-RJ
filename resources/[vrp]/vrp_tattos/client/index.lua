...
vSERVER = Tunnel.getInterface("vrp_tattos")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local cam = nil
...
function openTattooShop()
	...
	elseif GetEntityModel(ped) == GetHashKey("mp_f_freemode_01") then
		...
		SetPedComponentVariation(ped,5,-1,0,2)
		SetPedComponentVariation(ped,6,60,0,2)
		SetPedComponentVariation(ped,7,-1,0,2)
		SetPedComponentVariation(ped,8,15,0,2)
		SetPedComponentVariation(ped,9,-1,0,2)
		SetPedComponentVariation(ped,10,-1,0,2)
...
RegisterNUICallback('finishTattoos', function(data,cb)
	--[[ 
		Realizar um sistema para ver se ele tem dinheiro para comprar as tatuagens
	 ]]
...
-----------------------------------------------------------------------------------------------------------------------------------------
local locateTattoos = {
	{ 1322.93,-1652.29,52.27, true },
	{ -1154.42,-1425.9,4.95, true },
	{ 322.84,180.16,103.58, true },
	{ -3169.62,1075.8,20.83, true },
	{ 1864.07,3747.9,33.03, true },
	{ -293.57,6199.85,31.48, true },

	{ -1620.5,-994.68,13.11, false },
	{ -1700.54,-955.74,7.77, false },
...
local blips = {}
function criarBlip()
    for k, v in pairs(locateTattoos) do
		if v[4] then
			blips[k] = AddBlipForCoord(v[1],v[2],v[3])
			SetBlipSprite(blips[k], 75)
			SetBlipColour(blips[k], 48)
			SetBlipScale(blips[k], 0.4)
			SetBlipAsShortRange(blips[k], true)
			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString("Estudio de Tatuagem")
