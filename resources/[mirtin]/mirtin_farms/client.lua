					...
					if distance <= 15.0 then
						...
						if distance <= 2.0 then
							if IsControlJustReleased(1, 51) and segundos <= 0 and not IsPedInAnyVehicle(PlayerPedId()) then 
								...
								if vSERVER.giveItem(itemRoute, parseInt(itemAmountRoute), parseInt(itemNumRoute),bancadaName) then
...
local locations = {
	[1] = { coords = vec3(982.99,-134.46,74.05), range = 10.0, permission = "perm.desmanche" }, -- hells
	[2] = { coords = vec3(-1140.23,-1689.27,4.75), range = 10.0, permission = "perm.desmanche" }, -- cdd
	[3] = { coords = vec3(735.68,-1072.95,22.16), range = 10.0, permission = "perm.desmanche" }, -- lacoste
	[4] = { coords = vec3(-2378.5,-219.09,31.37), range = 10.0, permission = "perm.desmanche" }, -- rocinha
	[5] = { coords = vec3(1515.62,-2099.09,76.8), range = 10.0, permission = "perm.desmanche" }, -- pedreira
