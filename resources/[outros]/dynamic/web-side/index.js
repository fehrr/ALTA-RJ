$(document).ready(function (){
	...
	window.addEventListener("message",function(event){
		...
		if(item["addbutton"] == true){
			...
			var aa = (`<button data-menu="`+item["menuid"]+`"class="b btn"><div class="title">`+item["title"]+`</div><div class="description" >`+item["description"]+`</div><i class="fas fa-chevron-right icon" style="float:right;margin-top: 9px;"></i></button>`)
			$(".container").append(aa);
			$(".container").show();
			submenus.push(aa);
		}
	...
	function goback(){
		var gobackbutton = (`<button style="background: #1e4eff;" id="goback" class="btn"><div class="title">Voltar</div><i class="fas fa-chevron-left icon" style="top:40%;right: 20px"></i></button>`);
