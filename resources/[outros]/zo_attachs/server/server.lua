...
vCLIENT = Tunnel.getInterface("flow_attachs")
src = {}
Tunnel.bindInterface("flow_attachs", src)
Proxy.addInterface("flow_attachs", src)
...
Citizen.CreateThread(
    function()
        zof.prepare(
            ...
            [[            CREATE TABLE IF NOT EXISTS flow_attachs(                user_id INTEGER,                attachs text,                PRIMARY KEY (`user_id`) USING BTREE            )        ]]
        )
        zof.prepare("zo/check_exists_table_attachs", "SHOW TABLES LIKE 'flow_attachs'")
        zof.prepare("zo/insert_attachs", "INSERT INTO flow_attachs(user_id, attachs) VALUES(@user_id, @attachs)")
        zof.prepare("zo/update_attachs", "UPDATE flow_attachs SET attachs = @attachs WHERE user_id = @user_id")
        zof.prepare("zo/get_attachs", "SELECT * FROM flow_attachs WHERE user_id = @user_id")
        async(
            function()
